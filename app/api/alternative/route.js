...
import { buildAlternativePrompt } from "@/lib/prompts";
import { validateAlternativeResponse } from "@/lib/validation";
...
export async function POST(req) {
  ...
  try {
    const { system, prompt } = buildAlternativePrompt({ subject, grade, topic, material, misconception });
    const raw = await callClaude({ system, prompt, maxTokens: 900 });
    const json = extractJson(raw);
    const result = validateAlternativeResponse(json);
...
function errorMessageFor(err) {
  ...
  return "Something went wrong while checking for an alternative interpretation. Please try again.";
}
